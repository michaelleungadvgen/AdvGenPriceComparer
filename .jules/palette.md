...
**Action:** Always replace `<div>` elements used for navigation with semantic `<a>` tags. Ensure the `href` uses an absolute path (e.g., `/item/{id}`) and apply utility classes like `text-decoration-none text-dark` to preserve visual styling.
