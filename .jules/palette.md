...
## 2024-05-24 - Semantic Links over Div OnClick
**Learning:** Using `<div>` with `@onclick` for navigation in Blazor breaks keyboard accessibility and native browser features (like middle-click).
**Action:** Always replace them with semantic `<a>` tags using `href` when linking to routes, applying utility classes like `text-decoration-none text-dark` and `style="display: block;"` to preserve visual styling.
