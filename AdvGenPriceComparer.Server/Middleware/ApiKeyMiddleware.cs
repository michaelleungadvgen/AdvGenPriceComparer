...
public class ApiKeyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IApiKeyService apiKeyService, IWebHostEnvironment env)
    {
        ...
        if (path.StartsWith("/swagger/") || path == "/swagger" ||
            path.StartsWith("/health/") || path == "/health" || path == "/")
        {
            await _next(context);
            return;
        }
        ...
        if (env.IsDevelopment() && context.Request.Method == "GET" &&
            (path.StartsWith("/api/prices/") || path == "/api/prices"))
        {
            // Public read access
            await _next(context);
            return;
